...
public:
  ...
  //optional, if read_queue_ is empty, return nullopt. If there is a value, return string.
  std::optional<std::string> read(){
    std::lock_guard<std::mutex> lock(read_mutex_);
    if(read_queue_.empty()) return std::nullopt;
...
private:
    ...
    boost::asio::async_connect(
        ...
        [this, self](boost::system::error_code ec, tcp::endpoint) {
          ...
          boost::asio::post(strand_,[this](){do_read();});
            ...
            std::cerr <<ERR<< " Client failed to connect. " <<ec.what()<< std::endl;
            do_close();
          }
        });
  }
  ...
  void do_read() {
    ...
    boost::asio::async_read_until(
        socket_,
        boost::asio::dynamic_buffer(read_buf_),
        '\n',
